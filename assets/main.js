...
const banner_v268_item_list = document.querySelectorAll('.banner-v268__item');
banner_v268_item_list.forEach((item, index) => {
    if (index === 0) {
        item.setAttribute("xo-active", "");
    }
    ;
    item.addEventListener('mouseover', () => {
        banner_v268_item_list.forEach((item) => item.removeAttribute('xo-active'));
        item.setAttribute("xo-active", "");
        const video_active = document.querySelector('.banner-v268__item[xo-active] video');
        const video_other_list = document.querySelectorAll('.banner-v268__item:not([xo-active]) video');
        if (video_active) {
            video_active.play();
        }
        video_other_list.forEach((video) => {
            video.pause();
        });
    });
});
...
// EXTERNAL MODULE: ./src/sections/banner-v2/banner-v2.script-global.ts
var banner_v2_script_global = __webpack_require__(402);
;// ./src/sections/slideshow-v4/constants.script-global.ts
const WebComponent = {
    SlideShow: 'slideshow-v1',
    SlideShowNext: 'slideshow-next',
    SlideShowPrev: 'slideshow-prev',
    SlideShowList: 'slideshow-list',
    SlideShowItem: 'slideshow-item',
    SlideShowThumbnail: 'slideshow-thumbnail',
    SlideShowThumbnailItem: 'slideshow-thumbnail-item',
};

;// ./src/sections/slideshow-v4/SlideShowStore.script-global.ts
...
;// ./src/sections/slideshow-v4/SlideShow.script-global.ts
...
class SlideShow extends HTMLElement {
    ...
    handleNext = () => {
        ...
        if (!isRunning) {
            const indexList = this.store.getState().index;
            this.slideList?.appendChild(this.slideItems[indexList]);
            this.store.increment();
            const indexThumbnail = this.store.getState().index;
            this.slideThumbnail?.appendChild(this.thumbnailItems[indexThumbnail]);
    ...
    handlePrev = () => {
        ...
        if (!isRunning) {
            ...
            const indexList = this.store.getState().index;
            this.slideList?.prepend(this.slideItems[indexList]);
            this.slideThumbnail?.prepend(this.thumbnailItems[indexThumbnail]);
    ...
    handleClick = (indexAt) => {
        switch (this.store.getState().index) {
            case 0:
                switch (indexAt) {
                    case 1:
                        this.handleNext();
    ...
    checkHasItems = () => {
        if (this.slideItems.length < 2) {
            this.nextEl?.setAttribute('hide', '');
            this.prevEl?.setAttribute('hide', '');
            this.slideThumbnail?.setAttribute('hide', '');
        }
    };
    bindEvents = () => {
        ...
        if (this.slideList) {
            this.pan = panGesture({
                element: this.slideList,
                onStart: () => {
                    console.log('Start');
                    this.pan?.setValue({ dx: 0 });
                },
                onEnd: ({ dx }) => {
                    console.log('End x', dx);
                    if (this.slideItems.length > 1 && dx > 0) {
                        this.handlePrev();
                    }
                    else if (this.slideItems.length > 1 && dx < 0) {
                        this.handleNext();
                    }
                },
            });
    ...
    initial = () => {
        this.nextEl = this.querySelector(WebComponent.SlideShowNext);
        this.prevEl = this.querySelector(WebComponent.SlideShowPrev);
        this.slideList = this.querySelector(WebComponent.SlideShowList);
        this.slideThumbnail = this.querySelector(WebComponent.SlideShowThumbnail);
        this.slideItems = this.querySelectorAll(WebComponent.SlideShowItem);
        this.thumbnailItems = this.querySelectorAll(WebComponent.SlideShowThumbnailItem);
        this.pan = null;
        this.store = new SlideShowStore(0);
    };
    checkThumbnailItems = () => {
        console.log(this.thumbnailItems.length);
        if (this.thumbnailItems.length > 1) {
            this.slideThumbnail?.appendChild(this.thumbnailItems[0]);
        }
    };
    connectedCallback() {
        this.initial();
        this.checkHasItems();
        this.store.updateMax(this.slideItems.length - 1);
        this.bindEvents();
        this.checkThumbnailItems();
    }
    disconnectedCallback() {
        this.nextEl?.removeEventListener('click', this.handleNext);
        this.prevEl?.removeEventListener('click', this.handlePrev);
        this.thumbnailItems?.forEach((item, i) => {
            item.removeEventListener('click', () => this.handleClick(i));
        });
        this.pan?.destroy();
    }
}

;// ./src/sections/slideshow-v4/index.script-global.ts




componentDefine({
    [WebComponent.SlideShow]: SlideShow,
});
...
const configureCore_WebComponent = {
...
class Xo360Viewer extends HTMLElement {
    ...
    appendImage = () => {
        ...
        const contentEl = this.querySelector(configureCore_WebComponent.Xo360ViewerContent);
        if (contentEl) {
    ...
    initial = () => {
        this.nextEl = this.querySelector(configureCore_WebComponent.Xo360ViewerNext);
        this.prevEl = this.querySelector(configureCore_WebComponent.Xo360ViewerPrev);
        this.playEl = this.querySelector(configureCore_WebComponent.Xo360ViewerPlay);
        this.pauseEl = this.querySelector(configureCore_WebComponent.Xo360ViewerPause);
        this.store = new Xo360ViewerStore(0);
        this.pan = null;
        attrBoolean.set(this, 'xo-loading', true);
    };
    renderContent = () => {
        const content = `<${configureCore_WebComponent.Xo360ViewerContent}></${configureCore_WebComponent.Xo360ViewerContent}>`;
...
componentDefine({
    [configureCore_WebComponent.Xo360Viewer]: Xo360Viewer,
});
